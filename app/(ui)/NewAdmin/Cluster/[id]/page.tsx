...
import { deleteSingleExam, fetchSingleCluster } from '@/app/lib/actions';
              ...
              {datas.exams.map((exam) => (
                  <tr className='min-[426px]:justify-around  flex bg-gray-100 py-2 w-full pr-2 items-center max-[425px]:gap-6' key={exam.id}>
                    ...
                    <Link href={`/NewAdmin/Exams/${exam.id}`} key={exam.id}>
                      ...
                      <td className='w-1/12 mx-4' >
                        <form action={deleteSingleExam} className='bg-rose-500 p-2 text-white text-sm lg:rounded-md w-full flex items-center justify-center rounded-full'>
                        <input type="text" hidden value={exam.id} name='projectId' />
                      <button>
                        <div>
